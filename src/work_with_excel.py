...
class ExcelTemplate(ExcelWriter):
    ...
    def add(self, workbook):
        ...
        if worksheet:  # Проверяем, существует ли лист
            worksheet.write('G4', 'Вставляем данные')  # Записываем пример данных в ячейку A1
    ...
    def configuration(self, workbook):
        ...
        if worksheet:  # Проверяем, существует ли лист
            ...
            worksheet.write_formula('C10', '=SUM(C3:C9)', workbook.add_format({'bold': True}))

            # Дополнительные настройки форматов, если необходимо
            currency_format = workbook.add_format({'num_format': '$#,##0'})
            worksheet.set_column('C:C', None, currency_format)  # Устанавливаем формат валюты для столбца C
        else:
            print("Лист 'Шаблон' не найден для настройки!")


# Пример использования
excel_template = ExcelTemplate()  # Создаем экземпляр класса ExcelTemplate
excel_template.write('../data/newdata/new_template_excel.xlsx')  # Записываем содержимое в новый файл
